...
For experimental evaluation, each CSV row also stores a paired vision-only
baseline computed from the exact same model probability stream, persistence
length and base threshold, but with the contextual gate forced open. This
allows direct measurement of alert decisions changed or suppressed by CAN
rules without a second run.
...
class ScenarioRunner:
    ...
    def __init__(self, dbc_path: str, channel: str, interface: str):
        ...
        # Metadata used only for experiment logging. It does not affect the
        # CAN logic or the scenario execution itself.
    ...
    def _run(self, scenario: dict, name: str, delay: float, loop: bool) -> None:
        ...
        while not self._stop.is_set():
            ...
            with self._meta_lock:
                self._cycle_started_at = cycle_start
    ...
    def snapshot(self, now: float | None = None) -> dict:
        """Return timing metadata for the injected scenario."""
        if now is None:
            now = time.perf_counter()
        with self._meta_lock:
            name = self._scenario_name
            started = self._scenario_started_at
            cycle_started = self._cycle_started_at
            cycle = self._cycle_index
            active = self._active

        return {
            "scenario_name": name,
            "scenario_active": int(active),
            "scenario_cycle": cycle if cycle >= 0 else "",
            "scenario_t_s": (
                round(now - cycle_started, 3)
                if cycle_started is not None else ""
            ),
            "scenario_total_t_s": (
                round(now - started, 3)
...
def parse_args():
    ...
    p.add_argument(
        "--csv",
        default=None,
        help="Per-iteration evaluation trace with paired vision-only and "
             "context-aware decisions, explicit rule flags, CAN state and "
...
def flags_from_reasons(reasons: list[str]) -> dict[str, int]:
    """Expand textual rule reasons into analysis-friendly binary columns."""
    flags = {
        name: int(any(reason.startswith(prefix) for reason in reasons))
        for name, prefix in RULE_PREFIXES.items()
    }
    flags["ctx_520_stale"] = int("ctx:0x520_stale" in reasons)
    flags["ctx_560_stale"] = int("ctx:0x560_stale" in reasons)
    return flags


CSV_COLUMNS = [
    # Time / synchronization
    "iteration", "t_video_s", "t_wall_s", "clock_drift_s",
    "skipped_since_prev", "decision_compute_ms",

    # Scenario metadata
    "scenario_name", "scenario_active", "scenario_cycle",
    "scenario_t_s", "scenario_total_t_s",

    # Reaching branch, kept for backward compatibility
    "face_label", "face_p_reaching", "face_raw_positive",
    "gate_reaching", "gate_reaching_suppressed",
    "face_context_input", "ma_reaching",
    "baseline_ma_reaching", "baseline_alert_reaching",
    "context_alert_reaching", "alert_suppressed_reaching",
    "alert_introduced_reaching", "decision_changed_reaching",

    # Final phone branch
    "body_label", "body_p_phone", "body_raw_positive",
    "gate_phone", "gate_phone_suppressed",
    "body_context_input", "ma_phone",
    "baseline_ma_phone", "baseline_alert_phone",
    "context_alert_phone", "alert_suppressed_phone",
    "alert_introduced_phone", "decision_changed_phone",

    # CAN state / covariates
    "can_speed", "can_reverse", "can_turn", "can_hazard",
    "can_traffic_stop", "can_autostop_enable",
    "can_raining", "can_wiper_speed", "can_low_beam",
    "can_sensitivity", "can_rx_count",

    # Explicit rule activation flags
    "rule_G1a", "rule_G1b", "rule_G1c", "rule_G1d", "rule_G2",
    "rule_R1", "rule_R2", "rule_M3", "rule_M4",
    "ctx_520_stale", "ctx_560_stale",
    "phone_suppress_rule_count", "phone_suppress_reasons",

    # Persistence and thresholds
    "persistence_n", "threshold_base", "threshold_eff",
    "can_reasons",

    # Aggregate paired decisions
    "baseline_alert", "baseline_alert_class",
    "alert", "alert_class",
]
...
def main() -> None:
    ...
    # Paired vision-only baseline: same probability stream, same persistence
    # length and same base threshold. Only the CAN gate/sensitivity are removed.
    ...
    try:
        while True:
            ...
            row["t_wall_s"] = round(now - start_wall, 3)
            row["clock_drift_s"] = round(row["t_video_s"] - row["t_wall_s"], 3)
            row["skipped_since_prev"] = skipped_since_prev
            skipped_since_prev = 0

            if scenario_runner:
                row.update(scenario_runner.snapshot(now))
            else:
                row["scenario_name"] = ""
                row["scenario_active"] = 0
                row["scenario_cycle"] = ""
                row["scenario_t_s"] = ""
                row["scenario_total_t_s"] = ""
            ...
            row["can_rx_count"] = getattr(context, "rx_count", "")
            row["can_reasons"] = " ".join(gate["reasons"])

            row.update(flags_from_reasons(gate["reasons"]))

            phone_suppress_reasons = [
                reason for reason in gate["reasons"]
                if reason.startswith(("G1a:", "G1b:", "G1c:", "G1d:", "G2:"))
            ]
            row["phone_suppress_rule_count"] = len(phone_suppress_reasons)
            row["phone_suppress_reasons"] = " ".join(phone_suppress_reasons)
            row["persistence_n"] = args.n_persist
            row["threshold_base"] = round(args.threshold, 4)

            alert_class = None
            baseline_alert_class = None

            for name, feed in feeds.items():
                model = feed.model_name  # 'reaching' or 'phone'
                label, _, _ = feed.infer()
                p = feed.p_distraction()
                gate_open = bool(gate[model])

                # Production context-aware path.
                mean, alert, effective = persistence[name].push(
                    p, gate_open, gate["sensitivity"]
                )

                # Paired vision-only baseline. Same p stream, same N and same
                # base threshold; only the contextual gate is forced open and
                # sensitivity is neutral.
                baseline_mean, baseline_alert, _ = baseline_persistence[name].push(
                    p, True, 1.0
                )

                context_input = p if gate_open else 0.0
                suppressed_alert = bool(baseline_alert and not alert)
                introduced_alert = bool(alert and not baseline_alert)

                row[f"{name}_label"] = label
                row[f"{name}_p_{model}"] = round(p, 4)
                row[f"{name}_raw_positive"] = int(p >= args.threshold)
                row[f"gate_{model}"] = int(gate_open)
                row[f"gate_{model}_suppressed"] = int(not gate_open)
                row[f"{name}_context_input"] = round(context_input, 4)
                row[f"ma_{model}"] = round(mean, 4)
                row[f"baseline_ma_{model}"] = round(baseline_mean, 4)
                row[f"baseline_alert_{model}"] = int(baseline_alert)
                row[f"context_alert_{model}"] = int(alert)
                row[f"alert_suppressed_{model}"] = int(suppressed_alert)
                row[f"alert_introduced_{model}"] = int(introduced_alert)
                row[f"decision_changed_{model}"] = int(
                    bool(baseline_alert) != bool(alert)
                )
                row["threshold_eff"] = round(effective, 4)

                if baseline_alert and baseline_alert_class is None:
                    baseline_alert_class = model
                if alert and alert_class is None:
                    alert_class = model

            row["baseline_alert"] = int(baseline_alert_class is not None)
            row["baseline_alert_class"] = baseline_alert_class or ""
            row["alert"] = int(alert_class is not None)
            row["alert_class"] = alert_class or ""
            row["decision_compute_ms"] = round(
                1000.0 * (time.perf_counter() - decision_started), 3
            )
            ...
            if args.verbose:
                ...
                for name, feed in feeds.items():
                    ...
                    parts.append(
                        ...
                        f"base={row[f'baseline_ma_{model}']:.3f} "
                        f"gate={'ok ' if gate[model] else 'SUP'} "
                        f"A={row[f'context_alert_{model}']}/"
                        f"B={row[f'baseline_alert_{model}']}"
                    )
