...
def _display_names(class_names, override=None):
    if override:
        names = [x.strip() for x in override.split(",") if x.strip()]
        if len(names) != len(class_names):
            raise SystemExit(
                f"--display-class-names contiene {len(names)} nombres, "
                f"pero la evaluacion tiene {len(class_names)} clases."
            )
        return names
    return list(class_names)


def save_confusion_matrix_plot(cm, class_names, title, output_path, normalize=False):
    """Save one thesis-ready confusion-matrix figure."""
    cm = np.asarray(cm)
    ...
    image = ax.imshow(data, aspect="equal")
    ...
    # Put the value inside each cell. For the normalized version, also include
    # the absolute count so the figure remains directly interpretable.
...
def save_confusion_outputs(cm, class_names, split, level, out_dir, display_names=None):
    """Save count/normalized matrices as PDF, PNG and CSV."""
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    split_slug = split.lower()
    level_slug = level.lower()
    names = _display_names(class_names, display_names)

    count_title = f"{split.title()} — {level.title()} confusion matrix"
    norm_title = f"{split.title()} — {level.title()} normalized confusion matrix"
    ...
    for ext in ("pdf", "png"):
        save_confusion_matrix_plot(
            ...
            names,
            count_title,
            out_dir / f"confusion_{split_slug}_{level_slug}_counts.{ext}",
            normalize=False,
        )
        save_confusion_matrix_plot(
            cm,
            names,
            norm_title,
            out_dir / f"confusion_{split_slug}_{level_slug}_normalized.{ext}",
            normalize=True,
        )

    # Also save the underlying values for traceability.
    count_csv = out_dir / f"confusion_{split_slug}_{level_slug}_counts.csv"
    norm_csv = out_dir / f"confusion_{split_slug}_{level_slug}_normalized.csv"
    ...
    with norm_csv.open("w", newline="", encoding="utf-8") as f:
        ...
        for name, row in zip(names, norm):
            writer.writerow([name] + [f"{x:.8f}" for x in row])


def _split_output_path(path, split, multi_split):
    """Append split name before the extension when evaluating >1 split."""
    if path is None:
        return None
    p = Path(path)
    if not multi_split:
        return p
    return p.with_name(f"{p.stem}_{split.lower()}{p.suffix}")
...
def main():
    ...
    parser.add_argument(
        ...
        help="Split a evaluar. BOTH ejecuta VALIDATION y TEST en la misma corrida.",
    )
    parser.add_argument(
        "--csv-out",
        default=None,
        help=(
            "Opcional: CSV con metricas por clase. Con --split BOTH se agrega "
            "_validation/_test al nombre."
        ),
    )
    parser.add_argument(
        "--segment-csv",
        ...
        help=(
            "CSV con una fila por segmento y probabilidades medias. Con "
            "--split BOTH se agrega _validation/_test al nombre."
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "Directorio donde guardar matrices de confusion en PDF/PNG/CSV. "
            "Se generan matrices de conteos y normalizadas, tanto a nivel "
            "window como segment."
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "Nombres mostrados en las figuras, separados por coma. "
            "Ejemplo para binary_phone: 'Normal Driving,Phone Use'. "
            "No modifica las etiquetas internas ni los CSV de predicciones."
        ),
    )
    parser.add_argument(
        "--label-mode",
    ...
    num_classes_ckpt = inferred.pop("num_classes")
    if num_classes_ckpt != conf.num_classes:
        print(
            f"AVISO: el checkpoint tiene {num_classes_ckpt} clases pero config.yaml "
    ...
    modo = args.label_mode or ("fine" if num_classes_ckpt > 3 else "macro")
    ...
    # Load the model only once; reuse it for both splits.
    model = get_model(num_classes_ckpt, **inferred).to(args.device)
    missing, unexpected = model.load_state_dict(state_dict, strict=False)
    if missing or unexpected:
        print(
            f"AVISO al cargar pesos -> faltantes: {missing} | "
    ...
    for split in splits:
        ...
        dataset = SegmentDataset(
            os.path.join(conf.data_dir, split),
        ...
        dataset_class_names = list(getattr(dataset, "class_names", CLASS_NAMES))
        eval_class_names = (
            list(MACRO_NAMES_FINE) if modo == "fine" else dataset_class_names
        )
        ...
        if modo == "fine":
            eval_num_classes = len(MACRO_NAMES_FINE)
            report_class_names = list(MACRO_NAMES_FINE)
        else:
            eval_num_classes = num_classes_ckpt
            report_class_names = list(eval_class_names)

        print(
            format_report(
                cm_window,
                f"NIVEL WINDOW  ({split})",
                report_class_names,
            )
        )
        print(
            format_report(
                cm_segment,
                f"NIVEL SEGMENTO  ({split})",
                report_class_names,
            )
        )

        if args.segment_csv:
            seg_path = _split_output_path(args.segment_csv, split, multi_split)
            write_segment_csv(seg_path, segment_records)

        if args.csv_out:
            metric_path = _split_output_path(args.csv_out, split, multi_split)
            names = (
                report_class_names
                + [f"class_{i}" for i in range(eval_num_classes)]
            )[:eval_num_classes]
            ...
            for nivel, cm in (
                ...
                for m, n in zip(per_class_metrics(cm), names):
                    rows.append(
                        f"{nivel},{n},{m['precision']:.6f},"
                        f"{m['recall']:.6f},{m['f1']:.6f},{m['support']}"
                    )
            ...
            print(f"\nMetricas por clase guardadas en: {metric_path}")
        ...
        if args.confusion_matrix_dir:
            save_confusion_outputs(
                ...
                display_names=args.display_class_names,
            )
            save_confusion_outputs(
                cm_segment,
                report_class_names,
            ...
            print(
                f"\nMatrices de confusion guardadas en: "
                f"{Path(args.confusion_matrix_dir).resolve()}"
            )
