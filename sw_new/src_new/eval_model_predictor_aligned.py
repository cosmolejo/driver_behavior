...
def main():
    ...
    parser.add_argument(
        "--inspect-segment",
        default=None,
        help="Texto a buscar en session_name o segment_name para imprimir sus probabilidades.",
    )
    parser.add_argument("--num-workers", type=int, default=4)
    args = parser.parse_args()

    conf = OmegaConf.load(args.config)
    # Carga EXACTAMENTE igual a demo.py.
    predictor = Predictor(
    ...
    # Metadata del checkpoint solo para reporting.
    ...
    print("Carga del modelo: Predictor (misma ruta que demo.py)")
    ...
    if args.inspect_segment:
        matches = [
            r for r in segment_records
            if args.inspect_segment in r.get("segment_name", "")
            or args.inspect_segment in r.get("session_name", "")
        ]
        print()
        print("=" * 72)
        print(f"INSPECCION: {args.inspect_segment!r}")
        print("=" * 72)
        if not matches:
            print("No se encontraron segmentos coincidentes.")
        else:
            for r in matches:
                probs = " ".join(
                    f"{k}={v:.8f}"
                ...
                print(
                    f"{r['session_name']} / {r['segment_name']} | "
                    f"true={r['true_class']} pred={r['pred_class']} | {probs}"
                )
