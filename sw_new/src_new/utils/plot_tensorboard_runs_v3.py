...
With --aggregate-x, repeated x values are averaged across replicate runs and
the figure shows +/- 1 sample standard deviation when at least two replicates
are available. The aggregated values are also written to
summary_vs_x_aggregated.csv.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "When --x-values contains repeated values, aggregate runs sharing "
            "the same x value into one point using the mean of the selected "
            "summary statistic. If two or more replicates exist, the plot also "
            "shows +/- 1 sample standard deviation."
        ),
    )
    p.add_argument(
        "--x-label",
...
def multi_run_figures(
    ...
    if x_values is not None:
        ...
        for x, (label, data) in zip(x_values, runs):
            ...
            if vals:
                raw_points.append(
                    (x, summary_stat(vals, stat, plateau), label)
                )
        ...
        if raw_points:
            ...
            if aggregate_x:
                # Group replicated runs that share the same x value.
                grouped = defaultdict(list)
                for x, y, label in raw_points:
                    grouped[x].append((y, label))
                ...
                for x in xs:
                    values = [y for y, _ in grouped[x] if math.isfinite(y)]
                    if not values:
                        ys.append(math.nan)
                        yerr.append(0.0)
                    ...
                    if len(values) >= 2:
                        variance = sum(
                            (v - y_mean) ** 2 for v in values
                        ) / (len(values) - 1)
                        yerr.append(math.sqrt(variance))
                    else:
                        yerr.append(0.0)
                ...
                # Save the aggregated values for reproducibility.
                agg_csv = outdir / "summary_vs_x_aggregated.csv"
                with agg_csv.open("w", newline="", encoding="utf-8") as f:
                    ...
                    writer.writeheader()
                    for x, y, err, n in zip(xs, ys, yerr, replicate_counts):
                        writer.writerow({
                            "x": x,
                            "mean": y,
                            "sample_std": err if n >= 2 else "",
                            "n_replicates": n,
                            "run_labels": " | ".join(
                                label for _, label in grouped[x]
                            ),
                        })
            else:
