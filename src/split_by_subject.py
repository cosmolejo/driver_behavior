...
# file_id final: dmd/gF/25/s3/gF_25_s3_<timestamp>_<camera>_240
# (la clase DMD hace split('_')[:-2] que NO elimina el prefijo dmd/gF/25/s3/)
SUBJECT_RE = re.compile(r'dmd/(\w+)/(\d+)/')


def subject_from_path(s: str) -> str | None:
    """Extrae 'grupo_sujeto' de cualquier string que contenga dmd/<grupo>/<sujeto>/..."""
    m = SUBJECT_RE.search(s)
    return f"{m.group(1)}_{m.group(2)}" if m else None


# Aliases para mantener legibilidad en el resto del código
subject_from_csv_path = subject_from_path
subject_from_file_id = subject_from_path
