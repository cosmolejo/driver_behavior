"""
Trainer para modelo de dos cámaras (face + body) del dataset DMD.
"""
import logging
...
def _confusion_matrix_figure(cm: np.ndarray, class_names: list, normalize: bool = True):
    """
    Genera la figura matplotlib de la matriz de confusión para
    writer.add_figure de TensorBoard. Si normalize=True, normaliza por
    fila para porcentajes de recall por clase.
    """
    import matplotlib
    matplotlib.use('Agg')
...
class MultiCamTrainer(BaseTrainer):
    """
    Trainer para modelos que reciben dos streams de video (face + body)
    simultáneamente. Asume que el dataset devuelve tuplas
    (face_clip, body_clip, label) en modo multicámara, y que el modelo
    acepta dos tensores como input: model(face, body) -> logits.
    """
    ...
    def train_one_epoch(self, epoch):
        ...
        # Dataset multicámara devuelve (face_clip, body_clip, label)
        for batch_idx, (data_face, data_body, target) in enumerate(self.train_loader):
            data_face = data_face.to(self.device)
            data_body = data_body.to(self.device)
            ...
            if batch_idx % self.config.log_interval == 0:
                self.logger.info(
                    ...
                    f'[{batch_idx * len(data_face)}/{len(self.train_loader.dataset)} '
        ...
        # FIX: dividir por número de SAMPLES, no de batches
    ...
    def _evaluate(self, loader, split_name: str):
        """Lógica común para evaluación. Devuelve dict con métricas + arrays."""
        self.model.eval()
        total_loss = 0.0
        correct = 0
        all_preds = []
        ...
        with torch.no_grad():
            for data_face, data_body, target in tqdm(loader, desc=f"Eval {split_name}"):
                data_face = data_face.to(self.device)
                data_body = data_body.to(self.device)
                target = target.to(self.device)

                output = self.model(data_face, data_body)
                total_loss += self.loss(output, target).item() * data_face.size(0)
    ...
    def _log_evaluation(self, metrics: dict, split_name: str):
        """Loguea métricas + matriz de confusión + métricas por clase."""
        step = self.current_epoch
        cm = metrics['confusion_matrix']
        all_preds = metrics['all_preds']
        all_targets = metrics['all_targets']

        self.writer.add_scalar(f"Loss/{split_name}", metrics['loss'], step)
        self.writer.add_scalar("Metrics/Accuracy", metrics['accuracy'], step)
        ...
        self.logger.info(
            f"\n{split_name.capitalize()} set: Average loss: {metrics['loss']:.4f}, "
            f"Accuracy: {metrics['correct']}/{metrics['n_samples']} "
            f"({metrics['accuracy']:.0f}%), "
    ...
    def test(self):
        """Evaluación cíclica para selección de checkpoint y early stopping."""
    ...
    def validate(self):
        """
        Evaluación en held-out final (val_loader). Reservado para demos
        y comparación de modelos. Llamar manualmente.
        """
